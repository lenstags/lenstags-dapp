...
export const sendNotification = async (
  address: `0x${string}` | undefined | Array<`0x${string}` | undefined>,
  subject: NotificationTypes,
  profileName: string,
  target: number,
  titleContent?: string,
  profileId?: string
  ...
  if (target === NOTIFICATION_TYPE.SUBSET && Array.isArray(address)) {
    ...
    recipients = address.map(
      (addressFollower) => `eip155:80001:${addressFollower}`
    );
  ...
  const apiResponse = await PushAPI.payloads
    .sendNotification({
